  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      try {
        ...
        const response = await axios.post('https://vansh-2k25-welk.vercel.app/api/register', formData);
        ...
        if (response.data.acknowledgementId) {
          ...
          try {
            const pdfResponse = await axios.get(`https://vansh-2k25-welk.vercel.app/api/download-pdf/${response.data.formattedAckId}`, {
              responseType: 'blob'
            });
